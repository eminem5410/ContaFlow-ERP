  ...
  return (
    <Tabs defaultValue="balance-sheet" className="space-y-4">
      <TabsList className="bg-slate-100 flex-wrap h-auto gap-1">
        <TabsTrigger value="balance-sheet" className="text-sm">{t('reports.balanceSheet')}</TabsTrigger>
        <TabsTrigger value="income-statement" className="text-sm">{t('reports.incomeStatement')}</TabsTrigger>
        <TabsTrigger value="journal" className="text-sm">{t('reports.journal')}</TabsTrigger>
        <TabsTrigger value="ledger" className="text-sm">{t('reports.ledger')}</TabsTrigger>
        <TabsTrigger value="iva-sales" className="text-sm">{t('reports.ivaSales')}</TabsTrigger>
        <TabsTrigger value="iva-purchases" className="text-sm">{t('reports.ivaPurchases')}</TabsTrigger>
      </TabsList>
...
function BalanceSheet({ companyId }: { companyId: string }) {
  ...
  if (!data) return <p className="text-slate-400">{t('common.noData')}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">{t('reports.balanceSheet')}</h2>
          <p className="text-sm text-slate-500">{t('reports.asOf')} {data.asOf}</p>
        </div>
        {canExport('reports') && (
      ...
      <Card id="balance-sheet-report">
        <CardContent className="p-0">
          <div className="grid grid-cols-2 divide-x divide-slate-200">
            ...
            <div className="p-4 md:p-6">
              ...
              <div className="mt-4 pt-3 border-t-2 border-slate-300">
                <div className="flex justify-between text-base font-bold">
                  <span>{t('reports.totalActivos')}</span>
            ...
            <div className="p-4 md:p-6">
              <div className="mb-4">
                ...
                <div className="mt-3 pt-2 border-t border-slate-200">
                  <div className="flex justify-between text-sm font-bold">
                    <span>{t('reports.totalPasivos')}</span>
              ...
              <div>
                ...
                <div className="mt-3 pt-2 border-t border-slate-200">
                  <div className="flex justify-between text-sm font-bold">
                    <span>{t('reports.totalPatrimonio')}</span>
              ...
              <div className="mt-4 pt-3 border-t-2 border-slate-300">
                <div className="flex justify-between text-base font-bold">
                  <span>{t('reports.totalPasPatrim')}</span>
          ...
          <div className={`p-4 text-center text-sm font-medium border-t-2 ${data.balanced ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-red-50 border-red-300 text-red-700'}`}>
            {data.balanced ? (
              <><CheckCircle2 className="inline h-4 w-4 mr-1.5" /> {t('reports.balanceVerified')}</>
            ) : (
              <><XCircle className="inline h-4 w-4 mr-1.5" /> {t('reports.balanceNotVerified')}</>
            )}
          </div>
        </CardContent>
      </Card>
...
function IncomeStatement({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">{t('reports.incomeStatement')}</h2>
          <p className="text-sm text-slate-500">{t('reports.currentPeriod')}</p>
        </div>
        {canExport('reports') && (
          <div className="flex items-center gap-2">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-1.5 ml-3">
              ...
              <div className="flex justify-between text-sm font-bold pt-2 border-t border-slate-200">
                <span>{t('reports.totalIngresos')}</span>
          ...
          {costoVentas > 0 && (
            <div>
              ...
              <div className="space-y-1.5 ml-3">
                ...
                <div className="flex justify-between text-sm font-bold pt-2 border-t border-slate-200">
                  <span>{t('reports.totalCostoVentas')}</span>
          ...
          <div className={`p-3 rounded-lg border-2 ${resultadoBruto >= 0 ? 'bg-emerald-50 border-emerald-300' : 'bg-red-50 border-red-300'}`}>
            <div className="flex justify-between text-base font-bold">
              <span>{t('reports.resultadoBruto')}</span>
          ...
          {gastos > 0 && (
            <div>
              ...
              <div className="space-y-1.5 ml-3">
                ...
                <div className="flex justify-between text-sm font-bold pt-2 border-t border-slate-200">
                  <span>{t('reports.totalGastos')}</span>
          ...
          <div className={`p-4 rounded-lg border-2 ${resultadoNeto >= 0 ? 'bg-emerald-100 border-emerald-400' : 'bg-red-100 border-red-400'}`}>
            <div className="flex justify-between text-xl font-bold">
              <span>{t('reports.resultadoNeto')}</span>
            ...
            <p className={`text-sm mt-1 ${resultadoNeto >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
              {resultadoNeto >= 0 ? t('reports.gain') : t('reports.loss')}
            </p>
          </div>
        </CardContent>
      </Card>
...
function JournalReport({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">{t('reports.journal')}</h2>
        <p className="text-sm text-slate-500">{t('reports.journalSubtitle')}</p>
      </div>
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-24">{t('common.date')}</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-16">#</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('journalEntries.account')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.debit')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.credit')}</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
  ...
  return (
    <>
      ...
      <tr className="border-b border-slate-200">
        <td colSpan={3} className="px-4 py-2 text-right text-xs text-slate-500">{t('reports.totals')}</td>
...
function LedgerReport({ companyId }: { companyId: string }) {
  ...
  // Show all accounts for ledger selection (level filter was excluding manually-created accounts)
  const leafAccounts = accountsData?.accounts?.sort((a, b) => a.code.localeCompare(b.code)) || []
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">{t('reports.ledger')}</h2>
        <p className="text-sm text-slate-500">{t('reports.ledgerSubtitle')}</p>
      </div>
      ...
      {selectedAccountId && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {t('reports.type')}: <Badge className={`text-[10px] px-2 py-0.5 ${getAccountTypeColor(ledgerData?.account?.type || '')}`}>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    ...
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('reports.concept')}</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.debit')}</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.credit')}</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.saldo')}</th>
                  </tr>
                </thead>
                <tbody>
                  {isLoading ? (
                    ...
                    <>
                      ...
                      <tr className="border-t-2 border-slate-300 bg-slate-50">
                        <td colSpan={3} className="px-4 py-2.5 text-sm font-bold text-slate-700 text-right">{t('reports.totals')}:</td>
...
function IVASalesReport({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">{t('reports.ivaSales')}</h2>
          <p className="text-sm text-slate-500">{t('reports.ivaSalesSubtitle')}</p>
        </div>
        {canExport('reports') && (
          <div className="flex items-center gap-2">
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-24">{t('common.date')}</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-16">#</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('reports.description')}</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('reports.client')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.net')}</th>
                  <th className="text-center px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-20">{t('reports.rate')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">IVA</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('common.total')}</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex justify-between text-lg font-bold">
              <span>{t('reports.totalIvaSales')}</span>
...
function IVAPurchasesReport({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">{t('reports.ivaPurchases')}</h2>
          <p className="text-sm text-slate-500">{t('reports.ivaPurchasesSubtitle')}</p>
        </div>
        {canExport('reports') && (
          <div className="flex items-center gap-2">
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-16">#</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('reports.description')}</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider">{t('reports.provider')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('reports.net')}</th>
                  <th className="text-center px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-20">{t('reports.rate')}</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">IVA</th>
                  <th className="text-right px-4 py-2.5 text-xs font-medium text-slate-500 uppercase tracking-wider w-28">{t('common.total')}</th>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">{t('reports.taxCredit21')}</span>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">{t('reports.taxCredit105')}</span>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">{t('reports.taxCredit27')}</span>
            ...
            <Separator className="my-4" />
            <div className="flex justify-between text-lg font-bold">
              <span>{t('reports.totalIvaPurchases')}</span>
              <span className="font-mono text-emerald-700">{formatCurrencyExact(data.totals.total)}</span>
            </div>
          </CardContent>
        </Card>
